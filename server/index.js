...
app.get("/api/get-header-data", async (req, res) => {
  try {
    const dbo = await connect();
    const headerData = await dbo.collection("headertable").find({}).toArray();
    close();
...
// const Order = mongoose.model("xlData", orderSchema);
...
app.post("/api/getPdfData", async (req, res) => {
  try {
    ...
    // close(); // Assuming close() function closes the database connection
...
app.put("/api/update-order/:orderId/:itemId", async (req, res) => {
  const { orderId, itemId } = req.params;
  const updatedData = req.body;

  console.log(`Updating item with ID: ${itemId} in order with ID: ${orderId}`);
  ...
  try {
    ...
    const updatedOrder = await OrderDetail.findOneAndUpdate(
      ...
      {
        $set: {
          "Items.$.Product": updatedData.Product,
          "Items.$.LineItem": updatedData.LineItem,
          "Items.$.Qty": updatedData.Qty,
          "Items.$.Amount": updatedData.Amount,
          "Items.$.Tax": updatedData.Tax,
          "Items.$.Total": updatedData.Total,
        },
      },
      { new: true }
    );
    ...
    close(); // Ensure the database connection is closed
  }
});
